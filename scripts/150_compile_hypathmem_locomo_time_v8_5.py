#!/usr/bin/env python3
"""Compile Qwen-bound LoCoMo Cat2 operands into fail-closed solver candidates."""
...
def load_base() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("hypathmem_time_base138", path)
...
def main() -> None:
    ...
    for source in extraction_payload["rows"]:
        ...
        for hypothesis in extraction.get("hypotheses") or []:
            ...
            normalized = [
                normalize_locomo_binding(binding, base.normalize_binding)
    ...
    payload = {
        "metadata": {
            ...
            "fallback": "frozen_D0",
            "supported_operations": ["date", "ordering", "recency", "elapsed", "duration"],
        },
        "summary": {
            "num_questions": len(rows),
            "identity_valid_hypothesis": sum(any(h["source_validation"].get("eligible_for_normalization") for h in row["hypotheses"]) for row in rows),
