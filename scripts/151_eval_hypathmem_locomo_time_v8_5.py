#!/usr/bin/env python3
"""Evaluate verified LoCoMo 8.5 solver candidates with frozen D0 fallback."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--compiled",
        type=Path,
        default=Path("outputs/reconstruction/hypathmem_locomo_8.5time_v1/cat2_compiled.json"),
    )
    parser.add_argument(
        "--verified",
        type=Path,
        default=Path("outputs/reconstruction/hypathmem_locomo_8.5time_v1/cat2_q4_verified.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/qa/hypathmem_locomo_8.5time_v1/cat2_verified_solver_judge_gpt4omini.json"),
    )
    ...
    for index, source in enumerate(compiled["rows"], start=1):
        ...
        if active:
            ...
            judged = judge.judge_with_metadata(
                str(source["question"]),
                str(source.get("gold_answer") or ""),
                prediction,
            )
            route = "8.5time_locomo_verified_solver"
        else:
        ...
        print(
            f"processed {index}/{len(compiled['rows'])} qid={question_id} route={route} "
            f"acc={payload['summary']['accuracy']:.4f} fix={payload['summary']['fix']} "
            f"break={payload['summary']['break']}",
            flush=True,
        )
